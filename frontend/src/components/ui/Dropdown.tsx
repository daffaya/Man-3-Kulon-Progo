...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  const buttonClasses = isMobile
    ? "flex items-center justify-between w-full px-2 py-3 text-left text-foreground font-medium"
    : "flex items-center space-x-1 font-medium transition-colors text-foreground hover:text-[rgb(var(--color-accent))]";

  const contentClasses = isMobile
    ? "w-full bg-background border-t border-[rgb(var(--color-border))]"
    : "absolute top-full left-0 mt-2 w-56 card shadow-lg py-1 ";

  const itemClasses = isMobile
    ? "block w-full py-3 text-foreground/90 hover:bg-[rgb(var(--color-secondary-hover))] text-sm"
    : "block py-2 text-sm text-foreground hover:bg-[rgb(var(--color-secondary-hover))]";

  return (
    <div className={isMobile ? "" : "relative"} ref={dropdownRef}>
      <button onClick={toggle} className={buttonClasses} aria-expanded={isOpen}>
